...
if( !class_exists( 'vx_crmperks_notice_vxcf_infusionsoft' )):

class vx_crmperks_notice_vxcf_infusionsoft extends vxcf_infusionsoft{
    
public $plugin_url="https://www.crmperks.com";
public $review_link='https://wordpress.org/support/plugin/cf7-infusionsoft/reviews/?filter=5#new-post';
...
<p><i class="fa fa-check"></i> View Infusionsoft status(sent or not sent) of any entry.</p>
<p><i class="fa fa-check"></i> Easily Resend any entry to Infusionsoft.</p>
...
$msg =sprintf(__('Want to save contact form 7 submissions? Manage contact form 7 entries , view infusionsoft status of any entry and Resend any entry to infusionsoft with free %sConatct Form Entries Plugin%s','crm-perks-forms'),'<a href="https://wordpress.org/plugins/contact-form-entries/" target="_blank">','</a>');
?>
...
public function review_notice() { 
 $install_time=get_option($this->id."_install_data");
   ...
   if(empty($install_time['time'])){
    ...
    $time=current_time( 'timestamp' , 1 )-(16*7200);
 ...
 if(!empty($install_time) && is_array($install_time) && !empty($install_time['time']) && empty($install_time['review_closed'])){
  ...
  <p><?php echo sprintf(esc_html__( 'You\'ve been using Contact Form 7 infusionsoft Plugin for some time now; we hope you love it!.%s If you do, please %s leave us a %s rating on WordPress.org%s to help us spread the word and boost our motivation.','contact-form-entries'),'<br/>','<a href="'.$this->review_link.'" target="_blank" rel="noopener noreferrer">','&#9733;&#9733;&#9733;&#9733;&#9733;','</a>'); ?></p>
...
<p><i class="fa fa-check"></i> Infusionsoft Contact Files.</p>
<p><i class="fa fa-check"></i> Infusionsoft Affiliates Tracking.</p>
<p><i class="fa fa-check"></i> Assign tags to Infusionsoft Contact based on user input.</p>
...
new vx_crmperks_notice_vxcf_infusionsoft();
endif;
