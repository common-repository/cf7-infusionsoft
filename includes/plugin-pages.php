...
if( !class_exists( 'vxcf_infusionsoft_pages' ) ) {
...
* @since       1.0.0
*/
class vxcf_infusionsoft_pages   extends vxcf_infusionsoft{
    public $ajax=false;
...
global $pagenow; 
 ...
 add_filter("admin_menu", array($this, 'create_menu'), 49);
...
public function add_bulk_send_crm_callcenter($list){ 
 $list['send_infusionsoft']=__('Send to Infusionsoft','contact-form-infusionsoft-crm');
  return $list;  
}
public function bulk_send_crm_callcenter($msg,$action,$ids,$type){ 
    if($action == 'send_infusionsoft'){
...
$entry['__vx_id']=$entry['id'];
        ...
        if(empty($notice)){
            $notice='Nothing Sent to Infusionsoft';
        }
...
$push=$this->push($entry_detail,$form,'add_note');
  ...
  ?>
  <div class="vx_row">
  <label><input type="checkbox" name="<?php echo esc_attr($this->id) ?>_send" value="yes"> <?php esc_html_e('Send to Infusionsoft','contact-form-infusionsoft-crm') ?></label>
  </div>
  <?php  
}
    /**
...
$lead['__vx_id']=$id;
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . esc_html__('Settings', 'contact-form-infusionsoft-crm') . '</a>' );
        }
        return $links;
   }
  /**
  ...
  */
  ...
  // Adding submenu if user has access
        $page_title =__('Infusionsoft for Contact Form','contact-form-infusionsoft-crm');
        $menu_title =__('Infusionsoft','contact-form-infusionsoft-crm');
...
$class='';
 if($res){
       $msg=__('Account Deleted Successfully','contact-form-infusionsoft-crm');
  $class='updated';   
 }else{
       $msg=__('Error While Removing Account','contact-form-infusionsoft-crm');
  $class='error';      
 }
...
die();
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to add token','contact-form-infusionsoft-crm');
...
$info=$api->handle_code($id);
  ...
  $msg=__('You do not have permissions to export logs','contact-form-infusionsoft-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  ...
  $fields=array(); $field_titles=array("#",__('Status','contact-form-infusionsoft-crm'),__('Infusionsoft ID','contact-form-infusionsoft-crm') ,__('Entry ID','contact-form-infusionsoft-crm'),__('Description','contact-form-infusionsoft-crm'),__('Time','contact-form-infusionsoft-crm'));
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to clear logs','contact-form-infusionsoft-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $data=$this->get_data_object();
  $clear=$data->clear_logs();
       ...
       $msg=__('Error While Clearing Infusionsoft Logs','contact-form-infusionsoft-crm');
      $level="error";
      if(!empty($clear)){
      $msg=__('Infusionsoft Logs Cleared Successfully','contact-form-infusionsoft-crm');   
      $level="updated";
      }
  ...
  //
  self::$tooltips = array(
 'vx_feed_name' =>  esc_html__('Enter feed name of your choice.', 'contact-form-infusionsoft-crm'),
  'vx_sel_object' => esc_html__('Select the Object to Create when a Form is Submitted.', 'contact-form-infusionsoft-crm'),
   'vx_sel_account' =>__('Select the Infusionsoft account you would like to export entries to.', 'contact-form-infusionsoft-crm'),
  'vx_sel_form' => esc_html__('Select the Contact Form you would like to integrate with Infusionsoft. Contacts generated by this form will be automatically added to your Infusionsoft account.', 'contact-form-infusionsoft-crm'),
  
  'vx_map_fields' => esc_html__('Associate your Infusionsoft fields to the appropriate Contact Form fields.', 'contact-form-infusionsoft-crm'),
  
  'vx_optin_condition' =>__('When the opt-in condition is enabled, form submissions will only be exported to Infusionsoft when the condition is met. When disabled all form submissions will be exported.', 'contact-form-infusionsoft-crm'),
  
  'vx_manual_export' => esc_html__('If you do not want all entries sent to Infusionsoft, but only specific, approved entries, check this box. To manually send an entry to Infusionsoft, go to Entries, choose the entry you would like to send to Infusionsoft, and then click the "Send to Infusionsoft" button.', 'contact-form-infusionsoft-crm'),
  
    'vx_entry_notes' => esc_html__('Enable this option if you want to synchronize Contact Form entry notes to Infusionsoft Object notes. For example , when you add a note to a Contact Form entry, it will be added to the Infusionsoft Object selected in the feed.', 'contact-form-infusionsoft-crm'),
    
      'vx_primary_key' => esc_html__('Which field should be used to update existing objects?', 'contact-form-infusionsoft-crm'),
      
  'vx_oauth' => esc_html__('OAuth 2.0 is a industry-standard protocol for authorization', 'contact-form-infusionsoft-crm'),
  
  'vx_api' => esc_html__('Get Infusionsoft API key from Account Menu -> Integrations -> Get your API Key', 'contact-form-infusionsoft-crm'),
  
  'vx_custom_app'=>__('This option is for advanced users who want to override default Infusionsoft App.','contact-form-infusionsoft-crm'),
  
  'vx_disable_logs'=>__('When an order is sent to Infusionsoft we store that order information in the database and show it in the Infusionsoft Log. Check this box if you do not want to save the exported order information in the logs.','contact-form-infusionsoft-crm'),
  


  
   'vx_assign_company'=>__('Enable this option if you want to assign a Company this object.','contact-form-infusionsoft-crm'),
   'vx_sel_company'=>__('Select Contact feed. Company created by this feed will be assigned to this object.','contact-form-infusionsoft-crm'),
   
      'vx_assign_contact'=>__('Enable this option , if you want to assign a Contact to this object','contact-form-infusionsoft-crm'),
   'vx_sel_contact'=>__('Select Contact feed. Contact created by this feed will be assigned to this object','contact-form-infusionsoft-crm'),
   
         'vx_tag_check'=>__('Enable this option if you want to assign a tag to this object.','contact-forms-infusionsoft-crm'),
   'vx_tags'=>__('Get Tags list from Infusionsoft','contact-forms-infusionsoft-crm'),
    'vx_sel_tag'=>__('Select a tag.','contact-forms-infusionsoft-crm'),
 
 
   'vx_camp_check'=>__('If enabled, Lead/Contact will be added to selected Campaign','contact-form-infusionsoft-crm'),
   'vx_owner_check'=>__('Enable this option if you want to assign another object owner.','contact-form-infusionsoft-crm'),
   'vx_owners'=>__('Get Users list from Infusionsoft','contact-form-infusionsoft-crm'),
   'vx_order_notes'=>__('Enable this option if you want to synchronize WooCommerce Order notes to Infusionsoft Object notes. For example, when you add a note to a WooCommerce Order, it will be added to the Infusionsoft Object selected in the feed.','contact-form-infusionsoft-crm'),
   'vx_sel_owner'=>__('Select a user as a owner of this object','contact-form-infusionsoft-crm'),
   
         'vx_entry_note'=>__('Check this option if you want to send more data as CRM entry note.', 'contact-form-infusionsoft-crm'),
   'vx_note_fields'=>__('Select fields which you want to send as a note', 'contact-form-infusionsoft-crm'),
   'vx_disable_note'=>__('Enable this option if you want to add note only for new CRM entry', 'contact-form-infusionsoft-crm'),
...
<div class="table_head_i"><?php esc_html_e('Capsule', 'contact-form-infusionsoft-crm'); ?></div>  
<div class="vx_group">
...
echo $this->format_log_msg($log);
 ...
 $msg=__('You do not have permissions for this action','contact-form-infusionsoft-crm');
...
if( $this->has_feed($form_id)) { 
 $boxes['infusionsoft_crm']=array('title'=>'<i class="fa fa-plug"></i> '.__('Infusionsoft', 'contact-form-infusionsoft-crm'),'callback'=>array($this,'meta_box_html'));  
} return $boxes; 
}
...
public function has_feed($form_id) {
 ...
 if(!empty($info['meta']) ){
  ...
  case"refresh_users": 
    ...
    if(is_array($users)){
    $res['status']="ok";
    $data['crm_sel_user']=$this->gen_select($users,$owner_sel,__('Select User','contact-form-infusionsoft-crm'));
    $meta['users']=$users;   
    }else{
     $res['error']=$users;   
    }
...
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','contact-form-infusionsoft-crm');
    }
}
  die(json_encode($res));    
  }
  ...
  */
  public  function mapping_page(){
      ...
      wp_enqueue_style('vxc-tooltip');
         $tabs=array('feeds'=>__('Infusionsoft Feeds','contact-form-infusionsoft-crm') , 'logs'=>__('Infusionsoft Log','contact-form-infusionsoft-crm') , 'accounts'=>__('Infusionsoft Accounts','contact-form-infusionsoft-crm'), 'settings'=>__('Settings','contact-form-infusionsoft-crm'));
  ...
  $(this).text('<?php esc_html_e('Show Key','contact-form-infusionsoft-crm') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php esc_html_e('Hide Key','contact-form-infusionsoft-crm') ?>');  
...
start_tooltip();  
  ...
  */
  ...
  esc_html_e('You do not have permissions to access this page','contact-form-infusionsoft-crm');    
  return;
  }
  ...
  <p>
  <?php esc_html_e("Feed deleted.", 'contact-form-infusionsoft-crm') ?>
  </p>
  </div>
  <?php
  }
  ...
  <div class="updated fade" style="margin:10px 0;">
  <p>
  <?php esc_html_e("Feeds deleted.", 'contact-form-infusionsoft-crm') ?>
  </p>
  </div>
  <?php
  }
...
$count=$this->data->delete_log($log_ids);
  $this->screen_msg(sprintf(__('Successfully Deleted %d Item(s)','contact-form-infusionsoft-crm'),$count));  
    }
  ...
  if(!empty($entry) && is_array($entry)){ 
    ...
    $push=array('class'=>'error','msg'=>__('Entry Not Found','contact-form-infusionsoft-crm'));  
  }
  ...
  case"time": $time_order=$order_icon; $time_class="";   break;    
    ...
    $bulk_actions=array(""=>__('Bulk Action','contact-form-infusionsoft-crm'),"delete"=>__('Delete','contact-form-infusionsoft-crm'),
  'send_to_crm_bulk'=>__('Send to Infusionsoft','contact-form-infusionsoft-crm'),'send_to_crm_bulk_force'=>__('Force Send to Infusionsoft - Ignore filters','contact-form-infusionsoft-crm'));
  $base_url=$this->get_base_url();
 $objects=$this->get_objects();
      $statuses=array("1"=>__("Created",'contact-form-infusionsoft-crm'),"2"=>__("Updated",'contact-form-infusionsoft-crm'),"error"=>__("Failed",'contact-form-infusionsoft-crm'),"4"=>__("Filtered",'contact-form-infusionsoft-crm'),"5"=>__("Deleted",'contact-form-infusionsoft-crm')); 
...
foreach($map as $field_k=>$field_v){
  ...
  $sel_fields=array(""=>__("Standard Field",'contact-form-infusionsoft-crm'),"value"=>__("Custom Value",'contact-form-infusionsoft-crm'));
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = esc_html__('Infusionsoft','contact-form-infusionsoft-crm');
        return $plugins;
    }
  /**
...
$this->ajax=true;
  ...
  $msg=__("Please Choose Object",'contact-form-infusionsoft-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-infusionsoft-crm');
  }
  if($msg !=""){
  ?>
...
/*  $object=$this->post('object',$feed);
  ...
  $msg=__("Please Choose Account",'contact-form-infusionsoft-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-infusionsoft-crm');
  }
  if($msg !=""){
...
$field_options="<option>".esc_html__("Select Object",'contact-form-infusionsoft-crm')."</option>"; 
  if(is_array($objects)){
...
die();
  ...
  */
  ...
  if(!current_user_can($this->id.'_read_settings')){
  $msg_text=__('You do not have permissions to access this page','contact-form-infusionsoft-crm');   
  ...
  $this->uninstall();
  $uninstall_msg=sprintf(__("Contact Form Infusionsoft Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'contact-form-infusionsoft-crm'),"<a href='plugins.php'>","</a>");
$this->screen_msg($uninstall_msg);
  ...
  $meta=isset($_POST['meta']) ? $this->post('meta') : array();
   ...
   $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-infusionsoft-crm'));
...
include_once(self::$path . "templates/settings.php");
 ...
 $offset=$this->time_offset();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  esc_html_e('You do not have permissions to save settings','contact-form-infusionsoft-crm');
  return;   
  }
  $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-infusionsoft-crm'));
  $valid_email=true; $post=$crm=$this->post('crm');
  ...
  if(!$valid_email){
      $msgs['submit']=array("class"=>"error","msg"=>__('Invalid Email(s)','contact-form-infusionsoft-crm'));
  }
   $info=$this->get_info($id); 
...
//  $force_check=true;
  ...
  if($con_class != "updated" ){
      $msg=__('Connection to Infusionsoft is NOT Working','contact-form-infusionsoft-crm');  
  }else{
     $msg=__('Connection to Infusionsoft is Working','contact-form-infusionsoft-crm');   
  }
  $title=__('Test Connection: ','contact-form-infusionsoft-crm');
...
$msgs=array();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  esc_html_e('You do not have permissions to edit/save feed','contact-form-infusionsoft-crm'); 
  return;
  }
  //
...
if(is_array($feed_update) && is_array($feed)){
  ...
  if($is_valid){
      ...
      $msgs['save']=array('class'=>'updated','msg'=>sprintf(__("Feed Updated. %sback to list%s", 'contact-form-infusionsoft-crm'), '<a href="'.$feed_link.'">', "</a>"));
  }
  else{
  $msgs['save']=array('class'=>'error','msg'=>__("Feed could not be updated. Please enter all required information below.", 'contact-form-infusionsoft-crm'));
...
if(is_array($tags)){
  ...
  $fields['__vx_id']=array('id'=>'__vx_id','label'=>__('Entry ID','contact-form-infusionsoft-crm')); 
$fields['_vx_form_id']=array('id'=>'_vx_form_id','label'=>__('Form ID','contact-form-infusionsoft-crm')); 
$fields['_vx_form_name']=array('id'=>'_vx_form_title','label'=>__('Form Title','contact-form-infusionsoft-crm')); 

  $this->fields=$fields=array('cf'=>array("title"=>__('Contact Form Fields','contact-form-infusionsoft-crm'),"fields"=>$fields));
...
if( in_array($option_k,$sel_val)){
  ...
  */
  ...
  $modules=array(""=>__("Select Object",'contact-form-infusionsoft-crm'));
...
$info['valid_token']= !empty($info['class']) && $info['class'] == 'updated' ? 'true' : 'false';
  ...
  $msg=__( 'Successfully Connected to Infusionsoft','contact-form-infusionsoft-crm' );
     if(isset($info['_time'])){
